...
## 10. Дополнение после прогона #11 (2026-09-26): найден и исправлен баг пуша

- Первый dispatch (run #11) прошёл цикл корректно: expand **+400** (квота на стр. 82 favourites;
  онгоинги +135, анонсы +144, favourites +125; фильмы/ONA/OVA пропущены), enrich **395/400**,
  dates coverage **99.7%**, коммит `eaa0920` создан на раннере.
- Но в `main` он **не попал**: guard коммит-шага `git diff --cached --quiet || git push` после
  выполненного коммита всегда возвращает exit 0 (индекс чист) → `git push` не выполнялся никогда.
  Коммит остался на эфемерном раннере и потерян. Баг был скрыт: до #11 воркфлоу ни разу не
  доходил до коммита с изменениями.
- Фикс: проверка staged-изменений ДО коммита —
  `if ! git diff --cached --quiet; then git commit …; git push; fi`.
- Действие: повторный dispatch с теми же inputs (`deep=✅`, `max_new=400`): пулы целы
  (favourites стр. 83–100, онгоинги/анонсы), квота снова наберёт ровно 400.
