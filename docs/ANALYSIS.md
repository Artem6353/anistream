...
## 23. Итерация 3.6: история — одна карточка на тайтл, «Продолжить» в плеере и на странице тайтла, без «0:00 / 0:00» (25.09.2026)

**Проблема (сообщение пользователя).** В `/profile/history` дублировались карточки
одного тайтла: просмотр серий 1, 7, 8 «Табакошки» давал три отдельные карточки.
Плюс: для нативных источников (Demo, прямые HLS/MP4) известна позиция — надо
давать продолжить с того же места; для iframe (CVH/Kodik/AniBoom) `currentTime`
недоступен физически (позиция всегда 0) — такие записи показывались как
«0:00 / 0:00», что выглядело как «серия не смотрелась».

**Решения по задачам ТЗ.**

1. **Группировка в `/profile/history`** — `components/profile/HistoryList.tsx`:
   `latestBySlug()` (Map по slug, максимум `updatedAt ?? 0`, сортировка свежие
   сначала) — одна карточка на тайтл; `lib/library.ts` НЕ тронут (хранение по
   `(slug, episode)` осталось правильным источником). Карточка: постер,
   «Последняя серия: N» (фильмы — «Просмотрено»), `timeAgo`; при
   `position ≥ 1 && duration > 0` — бар + «MM:SS / MM:SS» + «Продолжить с
   MM:SS»; при `duration === 0` — «Смотрел N назад» без бара и без «0:00 /
   0:00». Клик → `/anime/{slug}/{episode}` последней серии. Удаление →
   `library.removeHistory(slug)` без episode (существующая семантика библиотеки:
   удаляет ВСЕ серии тайтла — проверено приёмкой A8). Добавлен CSS
   `.history__info` (колонка подписей; раньше title и meta слипались в одну
   строку) и `.history__resume`.
2. **Баннер «Продолжить» в плеере** — `components/player/PlayerShell.tsx`
   (только file-источники; для embed ветка не рендерит `<video>` вообще):
   в `onLoadedMetadata` читается запись истории текущего `(slug, episode)`;
   баннер показывается при `position ≥ 5 && position ≤ duration − 30`
   (без автоперемотки). Кнопки: «Продолжить» → `currentTime = position`,
   «Начать сначала» → `currentTime = 0`, обе скрывают баннер. Автоскрытие при
   `time > 10` (пользователь смотрит сам); сброс при смене серии/тайтла.
   Флаг `switchingSrc` подавляет баннер при смене озвучки — там позицию
   возвращает существующий `restore` по `keepTime` (без «мигания» баннера).
   CSS `.player__resume` (оверлей сверху по центру, как `.player__next`).
3. **Кнопка на странице тайтла** — новый клиентский компонент
   `components/anime/WatchButton.tsx` + замена статической кнопки в
   `app/anime/[slug]/page.tsx` (серверный компонент не видит localStorage):
   есть история → «Продолжить с серии N» (свежайшая запись slug по `updatedAt`)
   + маленькая ссылка «Смотреть с начала» → `/anime/{slug}/1`; нет истории →
   «Смотреть онлайн» как было; фильм (1 эпизод) с историей → «Продолжить
   просмотр». CSS `.watch-restart`. Кнопка в `EpisodeList` («Продолжить с
   серии N» в блоке серий) не тронута — дополняет, не конфликтует.
4. **Без «0:00 / 0:00»** — в `HistoryList` (см. 1) и в рейле: `PosterCard`
   получил опциональный `resumeNote` (замена подписи), `ContinueWatchingRail`
   передаёт `«Открыто {timeAgo}»` для записей с `duration === 0` вместо
   «Продолжить с серии N». **Группировка рейла (итерация 3.5) не менялась** —
   в рейле только +1 import и +1 display-prop (ТЗ задачи 4 прямо включает рейл
   в объём; запрет «не менять ContinueWatchingRail» относится к группировке).
   Прогресс-бар у iframe-карточек и раньше не рисовался (`progress > 0` guard).

**Приёмка (Playwright headless-shell, стенд: prod-сборка + `npm start`,
демо-MP4 Sintel 52 c через route-перехват, providers/availability застаблены;
«Табакошка» = slug `yani-neko`, 12 серий): 33/33 PASS, pageerror 0.**
A1–A8: 5 записей (3×yani-neko ep1/7/8 + frieren ep3 + one-piece ep5) → 3
карточки; Табакошка одна, «Последняя серия: 8», «Продолжить с 1:40», бар,
href `/anime/yani-neko/8`, клик → плеер «серия 8 из 12»; удаление стерло все
3 записи slug (остальные 2 целы). B1–B7: баннер «Продолжить с 0:20»
(duration≈52), «Продолжить» → currentTime≈20, «Начать сначала» → ≈0,
seek>10 → автоскрытие, position=40/52 (>duration−30) и position=3 (<5) →
баннера нет. C1–C2: embed-источник → баннера нет, iframe-фолбэк (итерация 3.3)
записал (0,0) без регрессии. D1–D6: «Продолжить с серии 8» + href ep8 +
«Смотреть с начала» → ep1, клик → ep8; без истории — «Смотреть онлайн» и нет
ссылки «с начала». E1–E9: iframe-строка «Последняя серия: 5 · Смотрел 2 часа
назад» без бара/«Продолжить»; нативная «5:00 / 20:00» + «Продолжить с 5:00»;
на странице истории и на главной нет «0:00 / 0:00»; рейл: one-piece →
«Открыто 2 часа назад» без бара, frieren → «Продолжить с серии 3» + бар.
F: `tsc --noEmit` чисто, vitest 26/26, `npm run build` успешно (49 c,
единственный warning — известный фреймворковый process.cwd/Edge Runtime),
`npm run smoke` — All routes OK.

**Файлы итерации:** `components/profile/HistoryList.tsx` (переписан),
`components/player/PlayerShell.tsx` (+баннер, ~60 строк),
`components/anime/WatchButton.tsx` (новый), `app/anime/[slug]/page.tsx`
(кнопка → WatchButton), `components/anime/PosterCard.tsx` (+resumeNote),
`components/anime/ContinueWatchingRail.tsx` (+2 display-строки),
